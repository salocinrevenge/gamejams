...
class Rain(BaseObject):
    ...
    def draw(self, surf: pygame.Surface):
        ...
        surf.blits([(self.img, self.img.get_rect(center=i.pos)) for i in self.particles])
...
class Window(BaseObject):
    ...
    def draw(self, surf: pygame.Surface):
        ...
        small_size = (max(1, self.s.get_width() // 8), max(1, self.s.get_height() // 8))
        blurred = pygame.transform.smoothscale(s, small_size)
        pygame.transform.smoothscale(blurred, self.s.get_size(), self.s)
